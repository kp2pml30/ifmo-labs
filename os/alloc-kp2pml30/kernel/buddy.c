...
// #define DEBUG_PRINT(A) printf(A); printf("\n");
...
bd_malloc(uint64 nbytes)
{
  ...
  bit_inv(bd_sizes[k].alloc, blk_index(k, p) / 2);
  for(; k > fk; k--) {
    ...
    bit_inv(bd_sizes[k-1].alloc, blk_index(k-1, p) / 2);
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_inv(bd_sizes[k].alloc, bi / 2);  // free p at size k
    // if (bit_isset(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
    if (bit_isset(bd_sizes[k].alloc, buddy / 2)) {  // is buddy allocated? == different
...
bd_initfree_pair(int k, int bi, void *bd_left, void *bd_right) {
  ...
  if(bit_isset(bd_sizes[k].alloc, bi / 2)) {
    ...
    */
    void *biaddr = addr(k, bi);
    void *buaddr = addr(k, buddy);
    if (bd_left <= buaddr && buaddr < bd_right)
      lst_push(&bd_sizes[k].free, buaddr);
    else
      lst_push(&bd_sizes[k].free, biaddr);
  }
  return free;
}
