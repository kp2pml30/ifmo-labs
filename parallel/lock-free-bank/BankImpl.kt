...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private fun acquire(index: Int, op: Op): AcquiredAccount? {
        ...
        assert(MAX_AMOUNT + MAX_AMOUNT > 0L)
        assert(-MAX_AMOUNT - MAX_AMOUNT < 0L)
        ...
        while (true) {
            ...
            if (account is AcquiredAccount) {
                if (op !== account.op && account.invokeOperation()) {
                    continue
                }
                return account
            }
    ...
    private inner class TotalAmountOp : Op() {
        ...
        override fun invokeOperation() {
            ...
            for (i in numberOfAccounts - 1 downTo  0) {
                release(i, this)
            }
        }
    }
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
             ...
             */
            assert(amount >= 0)
            val little = min(fromIndex, toIndex)
            // 🤓
            val big = little xor fromIndex xor toIndex

            try {
                val acL = acquire(little, this) ?: return
                val acB = acquire(big, this) ?: return
                val (acF, acT) = if (little == fromIndex) {
                    Pair(acL, acB)
                } else {
                    Pair(acB, acL)
                }
                when {
                    acF.amount - amount < 0L -> errorMessage = "Underflow"
                    acT.amount + amount > MAX_AMOUNT -> errorMessage = "Overflow"
                    else -> {
                        acF.newAmount = acF.amount - amount
                        acT.newAmount = acT.amount + amount
                    }
                }
                completed = true
            } finally {
