...
class TxVar<T>(initial: T)  {
    ...
    fun openIn(tx: Transaction, update: (T) -> T): T {
        ...
        while (true) {
            val curLoc = loc.value
            val curValue = valueIn(tx) { it.abort() }
            if (curValue === TxStatus.ACTIVE) {
                continue
            }
            val updValue = update(curValue as T)
            val newLoc = Loc(curValue, updValue, tx)
    ...
    /* now code in bank itself is easier, b/c we do not need to
     * acquire and release in inverse order
     */
}
