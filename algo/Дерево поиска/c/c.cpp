...
public:
  ...
  public:
    Node
      ...
      *left = nullptr,
      *right = nullptr;
    std::size_t key, czero;
    ValueType data;

    Node(const ValueType data) : key(1), data(data) { czero = data == 0; }
    ...
    void UpdateKey(void)
    {
      ...
      czero =
        + (data == 0 ? 1 : 0)
        + (left  == nullptr ? 0 : left ->czero)
        + (right == nullptr ? 0 : right->czero)
        ;
    }
    ...
    if (root->LeftEq() == key)
      ...
      else
      {
        ...
        if (r->czero != 0)
        {
          ...
          while (true)
          {
            if (zeroRemover->left != nullptr && zeroRemover->left->czero != 0)
              zeroRemover = zeroRemover->left;
            else if (zeroRemover->data == 0)
              break;
            else if (zeroRemover->right != nullptr)
              zeroRemover = zeroRemover->right;
            else
              break;
          }
          if (zeroRemover->data != 0)
            exit(50);
          Del(zeroRemover);
        }
      }
...
int main(void)
{
...
#endif
  ImplicitSplayTree<std::size_t> tree;
  std::size_t m = 0, n = 0;
  ...
  printf("%u\n", tree.root == nullptr ? 0 : tree.root->key);
  // nice O(n)
  std::size_t p;
  for (p = 1;; p++)
  {
    auto found = tree.Find(p);
    if (found == nullptr)
      break;
    printf("%u ", found->data);
  }
