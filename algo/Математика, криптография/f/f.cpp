...
#ifdef _DEBUG
auto cin = std::stringstream(R"delim(101010101
...
std::vector<int> mulpoly(std::vector<int> const& a, std::vector<int> const& b)
{
	...
	std::transform(fa.begin(), fa.end(), res.begin(), [](auto const& a) { return std::round(a.real()); });
	return res;
}
...
int main()
{
	...
	auto trees = std::vector<int>(n);

	std::transform(str.begin(), str.end(), trees.begin(), [](char c) { return c == '1'; });

	str.clear();
	str.shrink_to_fit();

	const auto b = mulpoly(trees, trees);
