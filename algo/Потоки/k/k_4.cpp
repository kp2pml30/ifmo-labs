...
int main()
{
	...
	{
		// volatile auto vec = std::vector<std::int8_t>(m * 1024 * 1024);
...
#if 0
	if (auto flow = g.Dinic(0, n + 1); flow != k)
	{
		cout << "-1" << std::endl;
		return 0;
	}
	...
	g.MinCost();
#else
	auto [flow, cost] = g.MinCostMaxFlow(0, n + 1);
	if (flow != k)
	{
		cout << "-1" << std::endl;
		return 0;
		...
		while (cur != n)
		{
			...
			for (auto const& b : g.verts[cur])
				if (auto eind = b.data; g.edgesData[eind].flow > 0
					&& bitch.count(g.edgesData[eind].id) == 0)
				{
					if (b.to != n + 1 && cur != 0)
					{
						if (std::abs(g.edgesData[eind].flow) != 1)
							throw "up";
						ans += std::abs(g.edgesData[eind].cost);
						accum.push_back(g.edgesData[eind].id);
						bitch.emplace(g.edgesData[eind].id);
					}
					cur = b.to;
					good = true;
					break;
	...
	{
		go();
		answer.push_back(accum);
	}

	cout << std::setprecision(5) << std::fixed << ans / k + (double)(ans % k) / k << '\n';

	if (answer.size() != k)
		throw "up";

	for (auto const& an : answer)
	{
		cout << an.size();
		for (auto const& a : an)
			cout << ' ' << a + 1;
