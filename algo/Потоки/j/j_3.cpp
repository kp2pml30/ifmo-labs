...
int main()
{
	...
	auto const& indF = [](int i) {
		return i;
	};

	auto const& indT = [n](int i) {
		return n + i;
	};
	...
	for (int i = 0; i < n; i++)
	{
		...
		ed.cost = 0;
		ed.minflow = 1;
		g.AddEdgeDirRaw(indF(i), indT(i), ed);
		ed.minflow = 0;
		ed.cost = co;
		g.AddEdgeDirRaw(indT(i), indF(i), ed);
	}
	...
	for (int i = 0; i < m; i++)
	{
		...
		ed.capacity = n;
		ed.cost = co;
		g.AddEdgeDirRaw(indT(f), indF(t), ed);
	}

	// cout << g << std::endl;

	auto [s, e, circ] = g.MakeCirculation();

	{
		EdgeData ed;
		ed.capacity = n;
		circ.AddEdgeDir(e, circ.verts.size() - 1, ed);
	}

	auto const [flow, cost] = circ.MinCostMaxFlow(s, circ.verts.size() - 1);

	// modern problems => modern solutions
	if (flow != n)
	{
		circ.ResetFlow();
		auto const flow = circ.Dinic(s, circ.verts.size() - 1);
		circ.MinCost();
		auto const cost = circ.EvalCost();
		if (flow != n)
			throw "up";
		cout << cost << std::endl;
		return 0;
