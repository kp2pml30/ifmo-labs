...
std::map<long long, std::vector<container>> query;
...
private:
	...
	type calc(uint l, uint r, uint x, uint lx, uint rx)
	{
		...
		if (lx >= l && rx <= r)
		{
			query[els[x].val].push_back({x, lx, rx});
			return els[x];
		}
		auto m = (lx + rx) / 2;
		return descr.oper(calc(l, r, 2 * x + 1, lx, m), calc(l, r, 2 * x + 2, m, rx));
...
public:
	...
	container taskj(container a, long long what)
	{
		while (a.x * 2 + 2 < els.size())
		{
			propagate(a.x, a.lx, a.rx);
			if (els[a.x * 2 + 1] == what)
			{
				a.lx = (a.lx + a.rx) / 2;
				a.x = a.x * 2 + 1;
...
int main(void)
{
	uint n, m;
	cin >> n >> m;
	MassTree<mindescr> tree(std::vector<setter>(n, 0));
	...
	while (!cin.eof())
	{
		...
		uint i, j;
		long long x;
		if (sscanf(str.c_str(), "defend %u %u %lld", &i, &j, &x) == 3)
		{
			i--;
			tree[{i, j}] = x;
		}
		else if (sscanf(str.c_str(), "attack %u %u", &i, &j) == 2)
		{
			i--;
			auto re = tree(i, j);
			auto &pp = query[re.val];
			if (pp.empty())
				throw "up";
			auto cont = pp.front();
			printf("%lld %u\n", re.val, tree.taskj(cont, re.val).x);
		}
		else
			return 0;
	}
