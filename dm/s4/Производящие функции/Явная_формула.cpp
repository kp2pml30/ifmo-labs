...
int main()
{
    ...
    auto makeUn_i = [&](std::int64_t i) {
        Poly u_n = {Rat(P(i).n, pow(r, i))}; //  * factorial<std::int64_t>[k]
        for (int j = 1; j <= k; j++)
            u_n *= Poly({j - i, 1});
        return u_n;
    };
    ...
    auto printSmart = [](auto const& poly, int desiredDeg) {
        if (poly.deg() > desiredDeg)
            throw "up";
        cout << poly;
    ...
    f_k *= Rat(1, factorial<std::int64_t>[k]);
